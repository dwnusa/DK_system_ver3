% 픽셀분할 영상처리
function [relabel_img, valid] = pixel_segmentation(IM_Sample, SIZX, SIZY)
sizx = SIZX; sizy = SIZY;
if sum(IM_Sample(:)) == 0
    relabel_img = zeros(sizx, sizy);
    valid = false;
    return;
end
% 임시 영상 생성
...
if max(v(:)) == 144 && size(x,1) == 144
    ...
    for ii = 1:12
        ...
        for jj = 1:12
            ...
            try
                temp2 = (img1_watershed == temp_label(1));
            catch
                valid = false;
                relabel_img = img1_watershed;
                return;
            end
