...
if(LUT_generator == 0)
    load('peak_data_07242122_LUT.mat');
end
multiplexing_type = 3;      % SCD 1, DPC 2, (201707)GC_ANALOGBOARD 3
...
fitting_lld = round(0.3*max(max(data)),-2);             % Starting point for searching maximum peak in photopeak region, % 15pF_1200, 30pF_1600, 51pF_1900
...
switch(multiplexing_type)
    ...
    case 3 % 2017.07 GC_ANALOGBOARD ver 
        for i=1:num
            rawdata(i,1)=round((image_size/2)*(data(i,2)-data(i,1))/(data(i,1)+data(i,2))) + (image_size/2);   % x-coordinate
            rawdata(i,2)=round((image_size/2) - (round(image_size/2)*(data(i,4)-data(i,3))/(data(i,3)+data(i,4))));  % y-coordinate
            rawdata(i,4)=(rawdata(i,2))+(image_size*rawdata(i,1))-1;                                                                  % pixel number
        end
end
...
if(LUT_generator == 1)
    ...
    I=(valid_count(:,:)-min(min(valid_count)))/(max(max(valid_count))-min(min(valid_count)))*256;
%     I=(valid_count(1:8,1:8)-min(min(valid_count(1:8,1:8))))/(max(max(valid_count(1:8,1:8)))-min(min(valid_count(1:8,1:8))))*256;
    figure;
    ...
    save([filename(1:18) '_count.mat'],'Ir','-mat');
end